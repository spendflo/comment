...
// scripts/validate-all-message-content.ts
...
// server/src/entity/message/MessageEntity.ts
var MessageEntity = class extends Model {
  isDeleted() {
...
import { Table as Table2, Column as Column2, PrimaryKey as PrimaryKey2, Model as Model2 } from "sequelize-typescript";
...
prom.collectDefaultMetrics({
  register,
  prefix: `${"scripts_validate_all_message_content"}_`
});
...
var defaultMeta = {
  process: "scripts_validate_all_message_content",
    ...
    if (sentryLogging && !process.env.IS_TEST) {
      ...
      if (sentryLevel) {
        const sentryEventID = Sentry.captureMessage(message, {
          ...
          tags: { loggingProcessName: "scripts_validate_all_message_content" },
...
var validateMessageContentAgainstSchema = ajv.compile(MessageAJVSchema);
...
async function main() {
  ...
  do {
    messages = await MessageEntity.findAll({
      where: {
        timestamp: {
          [Op3.gt]: messages.length > 0 ? messages[messages.length - 1].timestamp : CHECK_START
        }
      },
      order: [["timestamp", "ASC"]],
      limit: BATCH_SIZE
...
//# sourceMappingURL=validate-all-message-content.js.map
