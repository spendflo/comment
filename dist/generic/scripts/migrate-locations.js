...
prom.collectDefaultMetrics({
  register,
  prefix: `${"scripts_migrate_locations"}_`
});
...
var defaultMeta = {
  process: "scripts_migrate_locations",
    ...
    if (sentryLogging && !process.env.IS_TEST) {
      ...
      if (sentryLevel) {
        const sentryEventID = Sentry.captureMessage(message, {
          ...
          tags: { loggingProcessName: "scripts_migrate_locations" },
...
// scripts/lib/migrate_objects.ts
...
var uuidHash = (object) => uuidv5(jsonStableStringify3(object), RADICAL_UUID_NAMESPACE);
...
// scripts/lib/migrate_objects.ts
async function migrateObjects(orgID, sourceContextHash, destContext, transaction) {
...
// scripts/lib/util.ts
function maybePrintDryRunWarning({ commit }) {
  if (!commit) {
...
// scripts/migrate-locations.ts
var argv = yargs(process.argv.slice(2)).option("applicationID", {
  type: "string",
  ...
  demandOption: true
}).options(COMMIT_ARG).strict().help().alias("help", "h").argv;
async function main() {
  ...
  return await getSequelize().transaction(async (transaction) => {
    ...
    const transformation = JSON.parse(argv.transform);
    let count = 0;
    for (const page of pages) {
      ...
      if (!locationEqual(page.contextData, newContextData)) {
        console.log(
          `Moving ${(0, import_types.locationJson)(page.contextData)}
    => ${(0, import_types.locationJson)(
            newContextData
          )}`
        );
        if (argv.commit) {
...
function applyTransformation(context, transformation) {
...
//# sourceMappingURL=migrate-locations.js.map
