...
prom.collectDefaultMetrics({
  register,
  prefix: `${"scripts_resize_image_test"}_`
});
...
var defaultMeta = {
  process: "scripts_resize_image_test",
    ...
    if (sentryLogging && !process.env.IS_TEST) {
      ...
      if (sentryLevel) {
        const sentryEventID = Sentry.captureMessage(message, {
          ...
          tags: { loggingProcessName: "scripts_resize_image_test" },
...
async function getResizedImageURL(sourceURL, options = {}, maxAgeSeconds = 48 * 3600) {
  const size2 = options.size || 72;
  const variant = `source_resized_${size2}`;
  ...
  const outputImageBuffer = await sharp(sourceDownload).pipelineColourspace("rgb16").resize(size2, size2, { fit: "cover" }).rotate().jpeg({ quality: 90 }).toBuffer();
...
//# sourceMappingURL=resize-image-test.js.map
