...
prom.collectDefaultMetrics({
  register,
  prefix: `${"scripts_badge_image_test"}_`
});
...
var defaultMeta = {
  process: "scripts_badge_image_test",
    ...
    if (sentryLogging && !process.env.IS_TEST) {
      ...
      if (sentryLevel) {
        const sentryEventID = Sentry.captureMessage(message, {
          ...
          tags: { loggingProcessName: "scripts_badge_image_test" },
...
var getSignedUploadURL = (key, size2, mimeType, config) => getSignedURL(key, UPLOAD_URL_TTL_SECONDS, config, "PUT", {
...
var manageConnection = (url2) => {
  const httpAgent = new http.Agent();
  const httpsAgent = new https.Agent();
  const agent = url2.startsWith("https") ? httpsAgent : httpAgent;
  return requestFilterHandler(agent);
};

// server/src/util/safeFetch.ts
async function safeFetch(url2, init3) {
  return await nodeFetch(url2, { ...init3, agent: manageConnection(url2) });
}

// server/src/image_processing/util.ts
function downloadURL(url2) {
  return safeFetch(url2).then((response) => response.blob()).then((blob) => blob.arrayBuffer()).then((arrayBuffer) => Buffer.from(arrayBuffer));
}
var sha384 = (buffer) => crypto5.createHash("sha384").update(buffer).digest("hex");
...
async function getBadgedImageURL(sourceURL, badgeURL, options = {}, maxAgeSeconds = 48 * 3600) {
  const size2 = options.size || 112;
  const variantPrefix = `badge_${size2}_br:`;
  ...
  const sourceImage = sharp(sourceDownload).pipelineColourspace("rgb16").resize(size2, size2, { fit: "cover" }).rotate();
  ...
  const scaledBadgeWidth = Math.round(size2 * 0.4 * badgeInputAspectRatio);
  const scaledBadgeHeight = Math.round(size2 * 0.4 / badgeInputAspectRatio);
...
// scripts/badge-image-test.ts
var argv = yargs(process.argv.slice(2)).option("size", {
  type: "number",
...
var { size, inputUrl, badgeUrl } = argv;
async function main() {
...
//# sourceMappingURL=badge-image-test.js.map
