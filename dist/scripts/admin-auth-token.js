...
// scripts/admin-auth-token.ts
async function main() {
  const { argv } = yargs(process.argv.slice(2)).option("withBearer", {
    type: "boolean",
  ...
  const authToken = await fetchAuthToken(argv.tier);
  if (authToken) {
...
//# sourceMappingURL=admin-auth-token.js.map
