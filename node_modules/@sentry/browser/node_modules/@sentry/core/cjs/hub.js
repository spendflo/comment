...
`);
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
   _callExtensionMethod(method, ...args) {
    ...
    const sentry = carrier._CORDSNTRY;
 ...
 * it has an optional `_CORDSNTRY` property, which then in turn requires us to always perform an unnecessary check
 * at the call-site. We always access the carrier through this function, so we can guarantee that `_CORDSNTRY` is there.
 **/
function getMainCarrier() {
  utils.GLOBAL_OBJ._CORDSNTRY = utils.GLOBAL_OBJ._CORDSNTRY || {
    extensions: {},
    hub: undefined,
  };
...
function getCurrentHub() {
  ...
  if (registry._CORDSNTRY && registry._CORDSNTRY.acs) {
    const hub = registry._CORDSNTRY.acs.getCurrentHub();
...
function setAsyncContextStrategy(strategy) {
  // Get main carrier (global for every environment)
  const registry = getMainCarrier();
  registry._CORDSNTRY = registry._CORDSNTRY || {};
  registry._CORDSNTRY.acs = strategy;
}
...
function runWithAsyncContext(callback, options = {}) {
  ...
  if (registry._CORDSNTRY && registry._CORDSNTRY.acs) {
    return registry._CORDSNTRY.acs.runWithAsyncContext(callback, options);
  }
...
function hasHubOnCarrier(carrier) {
  return !!(carrier && carrier._CORDSNTRY && carrier._CORDSNTRY.hub);
}
...
/**
 * This will set passed {@link Hub} on the passed object's _CORDSNTRY.hub attribute
 * @param carrier object
 * @param hub Hub
...
function setHubOnCarrier(carrier, hub) {
  ...
  const _CORDSNTRY = (carrier._CORDSNTRY = carrier._CORDSNTRY || {});
  _CORDSNTRY.hub = hub;
  return true;
}
